...
if (-not $NoFetch) {
  if ($Peer) {
    Invoke-Step "1/11 抓取上游数据 + 逐期交叉核对 + 第二来源复核（$Peer）+ 写入" {
      node scripts/fetch-data.js --peer $Peer --apply
    }
  } else {
    Invoke-Step "1/11 抓取上游数据 + 逐期交叉核对 + 写入" {
...
Invoke-Step "2/11 多数据源合并与交叉核对（两源不一致即停线）" { node scripts/merge-sources.js --apply }
Invoke-Step "3/11 清洗数据（原始 → 标准格式）" { node scripts/prepare-data.js }
Invoke-Step "4/11 数据硬校验（格式 / 奖级全枚举 / 组合概率 / 分布 / 指纹）" { node scripts/verify-data.js }
Invoke-Step "5/11 重跑 Walk-Forward 回测与蒙特卡洛" { node src/build.js }
Invoke-Step "6/11 重新生成页面" {
...
Invoke-Step "7/11 页面内联脚本冒烟测试（模拟浏览器点击三个交互工具）" { node scripts/smoke-test-client.js }
Invoke-Step "8/11 工作流规则级校验（GitHub 专属规则，防非法 cron）" { node scripts/lint-workflows.js }
Invoke-Step "9/11 区间标定测试（守住置信区间口径，防过度标记）" { node scripts/test-interval-calibration.js }
Invoke-Step "10/11 排程语义校验真实性测试（注入错误，确认校验真的会红）" { node scripts/test-schedule-lint.js }
Invoke-Step "11/11 合规红线审计" { node scripts/audit-safety.js }
